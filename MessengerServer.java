...
public class MessengerServer extends Server {
  ...
  public void processNewConnection(String _clientIp, int _clientPort) {
    PacketWelcome welcome =
        new PacketWelcome("Willkommen auf dem Messenger-Server!");
    byte[] welcomeData =
  ...
  @Override
  public synchronized void processMessage(String _clientIp, int _clientPort,
                                          ByteBuffer _buffer) {
    ...
    if (!istTeilnehmerAngemeldet(_clientIp, _clientPort)) {
      switch (msgId) {
        case LOGIN -> {
          ...
          if (user.isEmpty()) {
            ...
            byte[] errorData = Packet.serialize(ServerToClient.getId(ServerToClient.ERROR), error);
            ...
            if (!Argon2.INSTANCE.verify(user.get().getPassword(),
              ...
              {
                PacketError error = new PacketError("Du bist bereits woanders eingeloggt.");
                byte[] errorData = Packet.serialize(ServerToClient.getId(ServerToClient.ERROR), error);
                send(_clientIp, _clientPort, errorData);
              }
              else {
                ...
                byte[] loginOkData = Packet.serialize(ServerToClient.getId(ServerToClient.LOGIN_OK), loginOk);
        ...
        case REGISTER -> {
          ...
          if (userRepository.createUser(name, Argon2.hash(password))) {
            ...
            PacketError error = new PacketError("Benutzer " + name + " erfolgreich erstellt");
            byte[] errorData = Packet.serialize(ServerToClient.getId(ServerToClient.ERROR), error);
            send(_clientIp, _clientPort, errorData);
          } else {
      ...
      switch (msgId) {
        ...
        case GIVE_ALL_MEMBER -> {
          PacketAllMembers allMembers = new PacketAllMembers(getAllMember().toArray(new String[0]));
          byte[] allMembersData = Packet.serialize(ServerToClient.getId(ServerToClient.ALL_MEMBERS), allMembers);
          send(_clientIp, _clientPort, allMembersData);
        }
        case SEND_NAME_TO_ALL -> {
          PacketAccess access = new PacketAccess(memberFromIpAndPort(_clientIp, _clientPort));
          byte[] accessData = Packet.serialize(ServerToClient.getId(ServerToClient.ACCESS), access);
          sendToAll(accessData);
        }
        case LOGOUT -> {
          PacketExit exit = new PacketExit(memberFromIpAndPort(_clientIp, _clientPort));
          byte[] exitData = Packet.serialize(ServerToClient.getId(ServerToClient.EXIT), exit);
          sendToAll(exitData);
        ...
        case MESSAGE -> {
          ...
          for (String s : receivers) {
            ...
            byte[] textData = Packet.serialize(
                ServerToClient.getId(ServerToClient.TEXT), text);
...
@Override
public synchronized void processClosingConnection(String _clientIp,
                                                  int _clientPort) {
...
private void meldeTeilnehmerAn(String _clientIp, int _clientPort,
                               String pName) {
